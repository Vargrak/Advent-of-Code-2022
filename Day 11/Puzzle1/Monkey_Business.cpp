...
class Monkey
{
    ...
    {
        this->items[item] = ceil(this->items[item] / 3);
    }
...
int main()
{
    ...
    //Play 20 turns
    for (int round_number = 0; round_number != 20; round_number++)
    {
        for (auto monkey : Monkies)
        {   
            monkey->play_turn();
    ...
    std::vector<int> inspected_x_times;
    for (auto monkey : Monkies)
    {
